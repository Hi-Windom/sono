...
def test_repair_flow():
    print("=" * 60)
    print("10MB MP3 + v3.2a 单轨修复全流程测试 (后端 API)")
    print("=" * 60)
    ...
    # 1. 单轨上传
    print("\n1. 单轨上传...")
    status, upload_data = upload_single(TEST_FILE)
    ...
    print(f"   文件大小: {upload_data.get('file_size', 0) / 1024 / 1024:.2f} MB")
    ...
    while time.time() - repair_start < max_wait:
        ...
        if progress != last_progress or step != last_step:
            print(f"   [{elapsed:6.1f}s] {task_status} - {progress_pct:5.1f}% - {step}")
            ...
            stuck_count += 1
            if stuck_count >= 10:
                print(f"   [{elapsed:6.1f}s] ⚠️  进度卡住 {stuck_count*3}s: status={task_status} progress={progress_pct:.1f}% step={step}")
        
        if task_status in ("completed", "error", "failed", "cancelled", "timeout"):
            if task_status == "completed":
                ...
                print(f"\n   ❌ 修复失败 (状态: {task_status})")
                print(f"   错误信息: {error}")
                print(f"   完整状态数据: {json.dumps(status_data, ensure_ascii=False, indent=4)}")
                return False
