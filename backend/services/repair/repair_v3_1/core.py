...
def _vocal_ai_repair_enhanced(y, sr, strength):
    if strength <= 0:
        return y
    if y.ndim == 1:
        y = y.reshape(1, -1)
        _vocal_ai_repair_enhanced(y, sr, strength)
        return y[0]
...
def _vocal_exciter(y, sr, amount):
    if amount <= 0:
        return y
    if y.ndim == 1:
        y = y.reshape(1, -1)
        _vocal_exciter(y, sr, amount)
        return y[0]

    nyq = sr / 2
    sos_bandpass = butter(4, [2000/nyq, min(8000, nyq*0.95)/nyq], btype='band', output='sos')
...
def _vocal_compressor(y, sr, amount):
    if amount <= 0:
        return y
    if y.ndim == 1:
        y = y.reshape(1, -1)
        _vocal_compressor(y, sr, amount)
        return y[0]

    attack_ms = 10.0
    release_ms = 100.0
    attack_gamma = np.exp(-1000.0 / (attack_ms * sr / HOP_LENGTH + 1e-10))
    release_gamma = np.exp(-1000.0 / (release_ms * sr / HOP_LENGTH + 1e-10))
...
def _vocal_de_esser_advanced(y, sr, amount):
    if amount <= 0:
        return y
    if y.ndim == 1:
        y = y.reshape(1, -1)
        _vocal_de_esser_advanced(y, sr, amount)
        return y[0]

    nyq = sr / 2
    sos_sibilance = butter(4, [4000/nyq, min(8000, nyq*0.95)/nyq], btype='band', output='sos')
    ...
    for ch in range(y.shape[0]):
        ...
        for i in range(n_frames):
            ...
            if ratio_val > threshold_ratio:
                attenuation = 1.0 - (ratio_val - threshold_ratio) * amount * 1.5
...
def process_vocal_track(y, sr, params):
    ...
    if params.get("de_esser_advanced", 0) > 0:
        y = _vocal_de_esser_advanced(y, sr, params["de_esser_advanced"])
    ...
    if params.get("ai_repair_enhanced", 0) > 0:
    ...
    if params.get("exciter", 0) > 0:
        y = _vocal_exciter(y, sr, params["exciter"])

    if params.get("compressor", 0) > 0:
        y = _vocal_compressor(y, sr, params["compressor"])
...
def repair_audio(input_path: str, output_path: str, params: dict, progress_callback=None) -> dict:
    ...
    if progress_callback:
        progress_callback(0.05, "v3.1 加载人声轨...")
    ...
    if progress_callback:
        progress_callback(0.10, "v3.1 加载伴奏轨...")
    ...
    if progress_callback:
        progress_callback(0.70, "v3.1 保存人声修复结果...")
    ...
    if progress_callback:
        progress_callback(0.75, "v3.1 保存伴奏修复结果...")
    ...
    if auto_mix:
        if progress_callback:
            progress_callback(0.80, "v3.1 混音...")
