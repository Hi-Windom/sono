...
def apply_spectral_group_a(y, sr, params, n_fft, hop_length, issues_found, music_type="generic"):
    ...
    crackle_added = "毛刺修复v5" in issues_found
    essing_added = "齿音抑制v5" in issues_found
    noise_added = "智能降噪v5" in issues_found
    ...
    for ch in range(y.shape[0]):
        ...
        if de_crackle > 0:
            _apply_de_crackle_v5_inplace(S, mag, sr, n_fft, hop_length, de_crackle)
            if not crackle_added:
                issues_found.append("毛刺修复v5")
                crackle_added = True
            mag = np.abs(S)

        if de_essing > 0:
            _apply_de_essing_v5_inplace(S, mag, sr, n_fft, hop_length, de_essing, music_type, type_params)
            if not essing_added:
                issues_found.append("齿音抑制v5")
                essing_added = True
            mag = np.abs(S)

        if noise_red > 0:
            _apply_noise_reduction_v5_inplace(S, mag, sr, n_fft, hop_length, noise_red, music_type, type_params)
            if not noise_added:
                issues_found.append("智能降噪v5")
                noise_added = True
...
def _apply_de_crackle_v5_inplace(S, mag, sr, n_fft, hop_length, intensity):
...
def _apply_de_essing_v5_inplace(S, mag, sr, n_fft, hop_length, intensity, music_type, type_params):
    freqs = fft_frequencies(sr=sr, n_fft=n_fft)
    ...
    centroid_threshold = 1.0 + np.std(centroid_ratio) * 0.9
    ...
    for low, high, weight in sibilant_bands:
        ...
        for j in np.where(sibilant_frames)[0]:
            ...
            reduction = max(reduction, 0.2)
            S[band_mask, j] *= reduction


def _apply_noise_reduction_v5_inplace(S, mag, sr, n_fft, hop_length, intensity, music_type, type_params):
    n_frames = mag.shape[1]
