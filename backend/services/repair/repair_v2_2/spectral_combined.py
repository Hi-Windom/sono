...
def apply_spectral_combined(y, sr, params, n_fft, hop_length, issues_found, music_type="generic"):
    """
    合并频谱处理 - 单次 STFTimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def apply_spectral_combined(y, sr, params, n_fft, hop_length, issues_found, music_type="generic"):
    ...
    de_essing = params.get("de_essing", 0)
    noise_red = params.get("noise_reduction", 0)
    harmonic_enhance = params.get("harmonic_enimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def apply_spectral_combined(y, sr, params, n_fft, hop_length, issues_found, music_type="generic"):
    ...
    for ch in range(n_channels):
        ...
        S = stft(data, n_fft=n_fft, hop_length=hop_length)
        mag = np.abs(S)
        n_frames = magimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def apply_spectral_combined(y, sr, params, n_fft, hop_length, issues_found, music_type="generic"):
    ...
    for ch in range(n_channels):
        ...
        if de_essing > 0:
            _fast_de_essingimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def apply_spectral_combined(y, sr, params, n_fft, hop_length, issues_found, music_type="generic"):
    ...
    for ch in range(n_channels):
        ...
        if de_essing > 0:
            _fast_de_essing(S, mag, freqs, frame_energy, de_essing, music_type, n_fimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def apply_spectral_combined(y, sr, params, n_fft, hop_length, issues_found, music_type="generic"):
    ...
    for ch in range(n_channels):
        ...
        if de_essing > 0:
            _fast_de_essing(S, mag, freqs, frame_energy, de_essing, music_type, n_frames)
            if not essing_added:
                issues_found.append("齿音抑制import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def apply_spectral_combined(y, sr, params, n_fft, hop_length, issues_found, music_type="generic"):
    ...
    for ch in range(n_channels):
        ...
        if de_crackle >import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def apply_spectral_combined(y, sr, params, n_fft, hop_length, issues_found, music_type="generic"):
    ...
    for ch in range(n_channels):
        ...
        if de_crackle > 0:
            _fast_de_crackle(S, mag, frame_energy, deimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def apply_spectral_combined(y, sr, params, n_fft, hop_length, issues_found, music_type="generic"):
    ...
    for ch in range(n_channels):
        ...
        if harmonic_enhance > 0:
            _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, harmonic_enhance, music_type, n_fimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def apply_spectral_combined(y, sr, params, n_fft, hop_length, issues_found, music_type="generic"):
    ...
    for ch in range(n_channels):
        ...
        if harmonic_enhance > 0:
            _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, harmonic_enhance, music_type, n_frames)
            if not enhance_added:
                issues_found.append("谐波增强v8import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def apply_spectral_combined(y, sr, params, n_fft, hop_length, issues_found, music_type="generic"):
    ...
    for ch in range(n_channels):
        ...
        if harmonic_richness > 0:
            _fast_harmonic_richness(Simport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def apply_spectral_combined(y, sr, params, n_fft, hop_length, issues_found, music_type="generic"):
    ...
    for ch in range(n_channels):
        ...
        if harmonic_richness > 0:
            _fast_harmonic_richness(S, mag, freqs, sr, n_fft, harmonic_richness, music_type,import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def apply_spectral_combined(y, sr, params, n_fft, hop_length, issues_found, music_type="generic"):
    ...
    for ch in range(n_channels):
        ...
        if harmonic_richness > 0:
            _fast_harmonic_richness(S, mag, freqs, sr, n_fft, harmonic_richness, music_type, n_frames)
            if not richness_added:
                issues_found.append("谐波丰富import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def apply_spectral_combined(y, sr, params, n_fft, hop_length, issues_found, music_type="generic"):
    ...
    for ch in range(n_channels):
        ...
        result[ch] = istftimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_noise_reduction(mag, n_frames, intensity, music_type):
    """快速降噪 - 纯 Python 实现"""
    noise_frames = max(1, nimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_noise_reduction(mag, n_frames, intensity, music_type):
    """快速降噪 - 纯 Python 实现"""
    noise_frames = max(1, n_frames // 20)
    noise_profile = np.mean(mag[:, :noise_fimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_noise_reduction(mag, n_frames, intensity, music_type):
    ...
    if music_type == "classicalimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_noise_reduction(mag, n_frames, intensity, music_type):
    ...
    if music_type == "classical":
        floor = 0.25
    elif music_type == "vocal":import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_noise_reduction(mag, n_frames, intensity, music_type):
    ...
    snr = (mag ** 2) / (noise_profile ** 2import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_noise_reduction(mag, n_frames, intensity, music_type):
    ...
    snr = (mag ** 2) / (noise_profile ** 2 + 1e-10)
    gain = snr / (snr +import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_noise_reduction(mag, n_frames, intensity, music_type):
    ...
    alpha = 0.8
    for i in range(1, n_frames):
        gain[:, i] = alpha * gain[:, i-1] + (1 - alpha) * gain[:, i]import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_essing(S, mag, freqs, frameimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_essing(S, mag, freqs, frame_energy, intensity, music_type, n_fimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_essing(S, mag, freqs, frame_energy, intensity, music_type, n_frames):
    """快速去齿音 - 纯 Python 实现"""
    centroid = np.sum(freqs[:, np.newaxis] * mag, axis=0) / (import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_essing(S, mag, freqs, frame_energy, intensity, music_type, n_frames):
    ...
    if n_frames >= 3:
        # 简单移动平均平滑
        centroid = np.convolve(centroid, [0.25, 0.5,import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_essing(S, mag, freqs, frame_energy, intensity, music_type, n_frames):
    ...
    mean_centroid = np.mean(centimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_essing(S, mag, freqs, frame_energy, intensity, music_type, n_frames):
    ...
    mean_centroid = np.mean(centroid)
    thr = mean_centroid * (1.2 + intensity * 0import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_essing(S, mag, freqs, frame_energy, intensity, music_type, n_frames):
    ...
    if not np.any(simport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_essing(S, mag, freqs, frame_energy, intensity, music_type, n_frames):
    ...
    if music_type == "vocal":
        mask = (freqs >= 3000) & (freqs <= 8000import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_essing(S, mag, freqs, frame_energy, intensity, music_type, n_frames):
    ...
    reduction = 1.0 - intensity * weight
    attenuation = np.ones(n_framesimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_essing(S, mag, freqs, frame_energy, intensity, music_type, n_frames):
    ...
    attenuation = np.ones(n_frames)
    attenuation[sibilant] = reduction
    S[mask, :] *= attenuationimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_crackle(S, mag, frameimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_crackle(S, mag, frame_energy, intensity, n_frames):
    """快速毛刺修复 - 纯 Pythonimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_crackle(S, mag, frame_energy, intensity, n_frames):
    ...
    # 简单移动平均平滑
    smooth_energy = np.convolve(frame_energy, [0.import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_crackle(S, mag, frame_energy, intensity, n_frames):
    ...
    # 简单移动平均平滑
    smooth_energy = np.convolve(frame_energy, [0.2, 0.6, 0.import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_crackle(S, mag, frame_energy, intensity, n_frames):
    ...
    ratio = frame_energy / (smooth_energy +import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_crackle(S, mag, frame_energy, intensity, n_frames):
    ...
    thr = np.mean(ratio) + np.std(ratioimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_crackle(S, mag, frame_energy, intensity, n_frames):
    ...
    if not np.anyimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_crackle(S, mag, frame_energy, intensity, n_frames):
    ...
    blend = intensity * 0.3
    phase = np.exp(1j * np.angleimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_crackle(S, mag, frame_energy, intensity, n_frames):
    ...
    for j in crackle_indices:
        left = max(0, j - 1)
        right = min(n_frames, j + 2)
        local_avg = np.mean(mimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_de_crackle(S, mag, frame_energy, intensity, n_frames):
    ...
    for j in crackle_indices:
        ...
        right = min(n_frames, j + 2)
        local_avg = np.mean(mag[:, left:right], axis=1, keepdims=True)
        mag[:, j] = local_avg[:, 0] * blend + mag[:, j] * (1 - blend)import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(Simport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_framesimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    if music_type == "vocalimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    if music_type == "vocal":
        base_freq_min, base_freq_max = 100, 200import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    if music_type == "vocal":
        base_freq_min, base_freq_max = 100, 2000
        harmonics = [(2, 0.06), (3, 0import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    if music_type == "vocal":
        ...
        base_freq_min, base_freq_max = 80import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    if music_type == "vocal":
        ...
        base_freq_min, base_freq_max = 80, 3000
        harmonics = [(2, 0.05), (3, 0.025)]import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    if music_type == "vocal":
        ...
        harmonics = [(2, 0.05), (3, 0.025)]
        max_harmonic_freq = 8000
    elif music_type == "import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    if music_type == "vocal":
        ...
        base_freq_min, base_freq_max = 80, 20import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    if music_type == "vocal":
        ...
        base_freq_min, base_freq_max = 80, 2000
        harmonics = [(2, 0.025), (3,import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    if music_type == "vocal":
        ...
        base_freq_min, base_freq_max = 80, 2000
        harmonics = [(2, 0.025), (3, 0.015)]
        max_harmonic_freq = 5000import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    if music_type == "vocal":
        ...
        base_freq_min, base_freq_max = 100,import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    if music_type == "vocal":
        ...
        base_freq_min, base_freq_max = 100, 2000
        harmonics = [(2, 0.035), (import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    if music_type == "vocal":
        ...
        base_freq_min, base_freq_max = 100, 2000
        harmonics = [(2, 0.035), (3, 0.02)]
        max_harmonic_freq = 600import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    base_mask = (freqs >= base_freq_min) & (freqs <= baseimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    harmonic_content = np.zeros_like(magimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    for h_num, h_gain_base in harmonics:
        ...
        for base_idx in base_indices[::2]:
            base_freq = freqs[base_idx]
            target_freq = base_freq * h_numimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    for h_num, h_gain_base in harmonics:
        ...
        for base_idx in base_indices[::2]:
            ...
            if target_freq >= min(max_harmonic_freq, nyquist - 100import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    for h_num, h_gain_base in harmonics:
        ...
        for base_idx in base_indices[::2]:
            ...
            target_idx = np.argmin(np.abs(freqs - target_freqimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    for h_num, h_gain_base in harmonics:
        ...
        for base_idx in base_indices[::2]:
            ...
            target_idx = np.argmin(np.abs(freqs - target_freq))
            base_mag = mag[baseimport numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_enhance(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    for h_num, h_gain_base in harmonics:
        ...
        for base_idx in base_indices[::2]:
            ...
            harmonic_mag =import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
...
def _fast_harmonic_richness(S, mag, freqs, sr, n_fft, intensity, music_type, n_frames):
    ...
    if music_type == "vocal":
        ...
        base_freq_min, base_freq_max = 100, 3500
        harmonics = [(2,import numpy as np
from services.librosa_compat import stft, istft, fft_frequencies
from .type_params import TYPE_PARAMS_MAP
