...
def _vocal_ai_repair_enhanced_lite(y, sr, strength):
    if strength <= 0:
        return y

    if y.ndim == 1:
        y = y.reshape(1, -1)
        _vocal_ai_repair_enhanced_lite(y, sr, strength)
        return y[0]
    ...
    for ch in range(y.shape[0]):
        ...
        local_medians = np.median(magnitude, axis=1, keepdims=True)
        threshold = local_medians * (1 + strength * 2.5)

        mask = np.ones_like(magnitude)
        below = magnitude < threshold
        threshold_broadcast = np.broadcast_to(threshold, magnitude.shape)
        mask[below] = magnitude[below] / (threshold_broadcast[below] + 1e-10)
...
def _vocal_exciter_lite(y, sr, amount):
    ...
    for ch in range(y.shape[0]):
        ...
        shaped = np.tanh(high_band * drive) * 0.5
        shaped = shaped + 0.3 * np.tanh(high_band * drive * 0.5)

        mix = amount * 0.4
        high_out = high_band * (1 - mix) + shaped * mix
        y[ch] = (low_band + high_out).astype(y.dtype)

    return y


def _vocal_compressor_lite(y, sr, amount):
    if amount <= 0:
        return y

    if y.ndim == 1:
        y = y.reshape(1, -1)
        _vocal_compressor_lite(y, sr, amount)
        return y[0]
    ...
    for ch in range(y.shape[0]):
        ...
        db_below_knee = db_thresh - knee
        db_above_knee = db_thresh + knee

        gain_db = np.zeros_like(db)
        below_knee = db < db_below_knee
        above_knee = db > db_above_knee
        in_knee = ~(below_knee | above_knee)

        gain_db[below_knee] = 0.0
        gain_db[above_knee] = (db_thresh - db[above_knee]) * (1 - 1 / ratio)

        if np.any(in_knee):
            knee_slope = (1 - 1 / ratio) / (4 * knee)
            gain_db[in_knee] = knee_slope * (db[in_knee] - db_below_knee) ** 2

        gain_linear = 10 ** (gain_db / 20.0)
        y[ch] = (data * gain_linear).astype(y.dtype)
...
def process_vocal_track(y, sr, params):
    ...
    if params.get("ai_repair_enhanced_lite", 0) > 0:
        y = _vocal_ai_repair_enhanced_lite(y, sr, params["ai_repair_enhanced_lite"])

    if params.get("exciter", 0) > 0:
        y = _vocal_exciter_lite(y, sr, params["exciter"])

    if params.get("compressor", 0) > 0:
        y = _vocal_compressor_lite(y, sr, params["compressor"])
...
def _repair_single_track(input_path: str, output_path: str, params: dict, progress_callback=None) -> dict:
    if progress_callback:
        progress_callback(0.05, "v3.1a 加载音频...")
    ...
    if progress_callback:
        progress_callback(0.10, "v3.1a 处理音频...")
    ...
    if single_params.get("ai_repair_enhanced_lite", 0) > 0:
        y = _vocal_ai_repair_enhanced_lite(y, sr, single_params["ai_repair_enhanced_lite"])

    if single_params.get("exciter", 0) > 0:
        y = _vocal_exciter_lite(y, sr, single_params["exciter"])

    if single_params.get("compressor", 0) > 0:
        y = _vocal_compressor_lite(y, sr, single_params["compressor"])
    ...
    if progress_callback:
        progress_callback(1.0, "v3.1a 修复完成")
...
def repair_audio(input_path: str, output_path: str, params: dict, progress_callback=None) -> dict:
    ...
    if progress_callback:
        progress_callback(0.05, "v3.1a 加载人声轨...")
    ...
    if progress_callback:
        progress_callback(0.10, "v3.1a 加载伴奏轨...")
    ...
    if progress_callback:
        progress_callback(0.70, "v3.1a 保存人声修复结果...")
    ...
    if progress_callback:
        progress_callback(0.75, "v3.1a 保存伴奏修复结果...")
    ...
    if progress_callback:
        progress_callback(0.80, "v3.1a 混音...")
    ...
    if mastering_style == "standard":
        if progress_callback:
            progress_callback(0.85, "v3.1a 标准母带...")
        mixed = _mastering_standard_lite(mixed, working_sr)
        issues_found.append("标准母带")
    elif mastering_style == "powerful":
        if progress_callback:
            progress_callback(0.85, "v3.1a 强劲母带...")
