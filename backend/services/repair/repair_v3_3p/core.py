...
def _repair_dual_track(input_path, output_path, params, progress_callback=None):
    ...
    vp = {
        ...
        "f0_harmonic": 0.7,
        "microtremor_breath": 0.5,
        "de_shimmer": 0.5,
        "phase_diffuse": 0.4,
        "noise_floor": 0.3,
    }
    for k, v in vocal_params.items():
        if k in vp or k in ("strength", "f0_guided_depth", "perceptual_weight"):
            vp[k] = v
    vp["strength"] = vocal_params.get("strength", params.get("strength", 1.0))

    ip = {
        "spectral_naturalize": 0.8,
        "noise_floor_shape": 0.0,
        "harmonic_deregularize": 0.6,
        "phase_naturalize": 0.0,
        "transient_protect": 0.5,
    ...
    if progress_callback:
        progress_callback(0.25, "v3.3+ 处理人声轨(增强)...")

    v_processed = process_vocal_v33(v_y, sr, vp)

    if progress_callback:
        progress_callback(0.50, "v3.3+ 处理伴奏轨...")

    a_processed = process_inst_v33(a_y, sr, ip)

    if progress_callback:
        progress_callback(0.70, "v3.3+ 混音...")
    ...
    mix_p = {
        ...
        "residual_refine": mix_params.get("residual_refine", 0.3),
    }
